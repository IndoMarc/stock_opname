...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>STOCK OPNAME - <?php echo $currentUser; ?></title>
    <script src="https://unpkg.com/html5-qrcode" type="text/javascript"></script>
    <style>
        :root { --primary: #2c3e50; --accent: #3498db; --danger: #e74c3c; --success: #27ae60; }
        body { font-family: sans-serif; margin: 15px; padding-bottom: 80px; background-color: #f8f9fa; }
        .header-top { display: flex; justify-content: space-between; align-items: center; color: var(--primary); margin-bottom: 15px; }
        .header-top h2 { margin: 0; font-size: 20px; }
        .btn-logout { background: var(--danger); color: white; text-decoration: none; padding: 8px 15px; border-radius: 5px; font-weight: bold; font-size: 12px; }
        ...
        .tab-menu { display: flex; position: fixed; bottom: 0; left: 0; width: 100%; background: white; border-top: 1px solid #ddd; z-index: 999; padding: 5px 0; }
        .tab-btn { flex: 1; padding: 12px 2px; cursor: pointer; background: transparent; border: none; font-size: 10px; font-weight: bold; color: #7f8c8d; transition: all 0.3s; text-align: center; }
...
<body>
    ...
    <div class="header-top">
        <h2>SO - <?php echo $currentUser; ?></h2>
        <a href="?logout=true" class="btn-logout">Logout</a>
    </div>

    <div id="tab0" class="tab-content active fade-in">
        <div class="filter-section">
            <h3 style="margin-top:0; color: var(--primary); text-align:center;">Sambung ke Wifi PC kasir lalu buka link ( <u>http://192.168.137.1:3000/api/load-data-so</u> ) dibrowser hp untuk download data stok</h3>
    ...
    <div id="tab3" class="tab-content">
        <div class="table-container">
            <table>
                <thead><tr><th>Modis</th><th>Shelf</th><th>Baris</th><th>PLU</th><th>Deskripsi</th><th>Harga</th><th>Stok LPP</th><th>Stok Fisik</th></tr></thead>
    ...
    <div id="tab4" class="tab-content">
        <div class="filter-section" style="display: flex; gap: 5px;">
            ...
            <button class="btn-cari" style="background-color: #27ae60;" onclick="copyAllResults()">Salin Semua Hasil</button>
    ...
    <div class="tab-menu">
        <button class="tab-btn active" id="btn0" onclick="switchTab(0)">DATA</button>
        <button class="tab-btn" id="btn1" onclick="switchTab(1)">PILIH</button>
        <button class="tab-btn" id="btn2" onclick="switchTab(2)" disabled>INPUT</button>
        <button class="tab-btn" id="btn3" onclick="switchTab(3)" disabled>DAFTAR</button>
        <button class="tab-btn" id="btn4" onclick="switchTab(4)" disabled>SELISIH</button>
    </div>
    ...
    <script>
            ...
            reader.onload = function(e) {
                try {
                    ...
                    if (result && result.data) {
                        ...
                        statusData.innerText = `Menggunakan data offline: ${file.name} (${result.data.length} item loaded)`;
        ...
        function renderTable() {
            ...
            document.getElementById('tableInput').innerHTML = sortedData.map(i => `<tr><td>${i.NAMA_RAK.substring(0,6)}</td><td>${i.NOSHELF}</td><td>${i.KIRIKANAN}</td><td>${i.PLUMD}</td><td>${i.DESC2}</td><td>${parseFloat(i.PRICE).toLocaleString('id-ID')}</td><td>${i.QTY}</td><td>${dataInputan.get(i.PLUMD) ?? ""}</td></tr>`).join('');
        }
